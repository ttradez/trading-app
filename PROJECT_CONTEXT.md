...
- **Onboarding rebuild — IN PROGRESS (2026-05-12).** Replacing the current `AccountSetup` / `Login` / `FeatureTour` flow with a 12-screen sequence informed by retention research. Source of truth: **`docs/ONBOARDING_RETENTION_RESEARCH.md`** (user is adding). Building one screen at a time, smoke-testing each before moving on.
  ...
  - **Follow-up tasks (deferred, must ship before launch):**
    ...
    2. **Streak system implementation** — *touches dashboard + main app, not just onboarding.* Screen 12 captures `dailyTimeGoalMinutes` (15/30/60/90/120/180). Still to build: (a) time-tracking inside the main app so we know how long the user actually trained today, (b) per-day streak increment when goal is hit, (c) streak reset on missed days, (d) streak counter on dashboard, (e) optional daily reminder notification scheduled around training time — that's where notifications would re-enter if we want them.

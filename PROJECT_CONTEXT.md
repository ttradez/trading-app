...
- **Onboarding rebuild — IN PROGRESS (2026-05-12).** Replacing the current `AccountSetup` / `Login` / `FeatureTour` flow with a 13-screen sequence informed by retention research. Source of truth: **`docs/ONBOARDING_RETENTION_RESEARCH.md`** (user is adding). Building one screen at a time, smoke-testing each before moving on.
  ...
  - **Follow-up tasks (deferred, must ship before launch):**
    ...
    2. **Streak system implementation** — *touches dashboard + main app, not just onboarding.* Screen 12 captures `dailyTimeGoalMinutes` (15/30/60/90/120/180). **Visual layer done 2026-05-14:** `StreakBadge` component (5 states — active / milestone / at_risk / frozen / broken + a `'new'` alias that renders as at_risk) lives at `src/components/StreakBadge.tsx`; `streakStore` (visual state only — `currentStreak`, `streakStatus`, `freezesRemaining: 2`, `lastCompletedDate`) lives at `src/store/streakStore.ts`; the badge is placed top-right of the dashboard header and a "Your streak starts today." teaser sits on screen 12 between the daily-goal card and the Enter app button. **Still to build:** (a) time-tracking inside the main app so we know how long the user actually trained today, (b) per-day streak increment when goal is hit, (c) at_risk → broken transition on missed days, (d) freeze consumption on a miss + freeze regeneration cadence, (e) milestone detection on day 7/14/30/60/100/365, (f) optional daily reminder notification scheduled around training time — that's where notifications would re-enter if we want them.
